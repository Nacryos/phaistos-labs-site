...
## 12. Revision 5 — soft ink-bleed strokes (2026-06-26)

The deferred "soft strokes" piece. WebGL 1px lines can't be soft, so rather than rewrite the intricate edge shader, a second **`THREE.Points` layer over the same edge geometry** stamps soft blots along every edge vertex (small custom shader: perspective-attenuated `gl_PointSize`, reuses `aColor`/`aSearchDim`, fades with the shared `uGlobalAlpha`). Lines now read as bleeding ink strokes with pigment pooling, while every existing line animation (cascade/fiber/pulse) and the search-dimming stay intact — the change is purely additive. Tuned to `uSize 18`, `uOpacity 0.30` (size 90 blobbed into watercolor splotches). The remaining PhaiPhon CTA was also removed from the homepage, leaving no PhaiPhon references on the page.
